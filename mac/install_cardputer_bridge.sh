...
# How often (seconds) the daemon refreshes the device usage dashboard via
# ccusage. Lower = more live, at the cost of more frequent ccusage runs.
CARDPUTER_USAGE_INTERVAL=1800
# Command the daemon runs to read usage (JSON on stdout). `--offline` uses
# ccusage's bundled pricing (no slow network fetch). If `ccusage` isn't on
# the daemon's minimal PATH, give an absolute path, e.g.
# /Users/you/.npm-global/bin/ccusage daily --json --offline
# Quote it — the value has spaces, and the installer `source`s this file.
CARDPUTER_CCUSAGE_CMD="ccusage daily --json --offline"
ENVSTUB
  chmod 600 "${ENV_PATH}"
  echo
...
CARDPUTER_USAGE_INTERVAL="${CARDPUTER_USAGE_INTERVAL:-1800}"
CARDPUTER_CCUSAGE_CMD="${CARDPUTER_CCUSAGE_CMD:-ccusage daily --json --offline}"
...
sed \
  ...
  -e "s#__CARDPUTER_USAGE_INTERVAL__#${CARDPUTER_USAGE_INTERVAL}#g" \
  -e "s#__CARDPUTER_CCUSAGE_CMD__#${CARDPUTER_CCUSAGE_CMD}#g" \
  "${PLIST_SRC}" > "${PLIST_DST}"
